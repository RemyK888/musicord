...
export class Musicord extends EventEmitter {
  ...
  constructor(client: Client, options?: MusicordOptions) {
    ...
    if(options && ('ytApiKey' in options || (<MusicordOptions><unknown>options).ytApiKey !== undefined)) this.options = options;
    this.client = client;
  ...
  public async play(song: string, channel: VoiceChannel | StageChannel | any) {
    ...
    const resource = createAudioResource((await createWritableStream('https://youtu.be/lTWZG_nIeb8')) as any);
